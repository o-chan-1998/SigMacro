...
Sub Plot()
    ...
    For iPlot = 0 To DATA_MAX_NUM_PLOTS - 1
        ...
        DebugMsg "currentColumn: " & currentColumn ' 19
        
        ' Create the plot if not exists
        If Not graphAlreadyExists And iPlot = 0 Then
           DebugMsg "creatingNewGraph..." ' This is actually called

           ' ' ----------------------------------------           
           ' ' This works as pasted from the working example
           ' ' ----------------------------------------           

           ' Redim PlotColumnCountArray(0)
           ' PlotColumnCountArray(0) = 2
           
           ' ActiveDocument.CurrentPageItem.CreateWizardGraph("Area Plot", "Simple Area", "XY Pair", ColumnsPerPlot, PlotColumnCountArray, "Worksheet Columns", "Standard Deviation", "Degrees", 0.000000, 360.000000, , "Standard Deviation", True)
...
Function _GetColumnMapping(plotType As String, currentColumn As Long) As Variant
    ...
    ' ' Debug: Output mapping values for inspection
    ' Dim mapOut As String
    ' mapOut = "Mapping for " & plotType & ":"
    ' For i = 0 To UBound(mapping, 2)
    '     mapOut = mapOut & vbCrLf & "Mapping[" & i & "]: Source Column = " & mapping(0, i) & ", Start Row = " & mapping(1, i) & ", End Row = " & mapping(2, i)
    ' Next i
    ' DebugMsg mapOut
...
Sub SetRanges()
    _SetXRange() ' This works
    _SetYRange() ' Why this does not work?
End Sub
