...
Function _GetColumnMapping(plotType As String, startCol As Long, endCol As Long) As Variant
    ...
    If (plotType <> "contour") And (plotType <> "conf_mat") Then
        nTailCols = 1
    Else
        nTailCols = 0
    End If
    ...
    For ii = 0 To UBound(mapping, 2)
        ...
        If (plotType = "contour") Or (plotType = "conf_mat") Then
            ' Use a smaller range for contour plots
            mapping(2, ii) = maxRow
        Else
            mapping(2, ii) = maxRow
        End If
...
Sub Plot()
    ...
    For iPlot = 0 To numPlots - 1
        ...
        ColumnsPerPlot = _GetColumnMapping(plotType, startCol, endCol)

        ' Get the column count array
        Dim PlotColumnCountArray() As Variant
        PlotColumnCountArray = _GetPlotCountColumnArray(plotType, startCol, endCol)
